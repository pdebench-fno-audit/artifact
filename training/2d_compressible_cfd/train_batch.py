"""
2D Compressible CFD — ALL 8 configurations (parameterized)
============================================================
Runs any of the 8 PDEBench 2D CFD configs with the same architecture as Test 29.
...
app = modal.App("fno-2dcfd-batch")
data_vol = modal.Volume.from_name("fno-2dcfd-data", create_if_missing=True)

# All 8 PDEBench 2D CFD configurations
CFD_CONFIGS = {
    "M01_Eta001": {
        ...
        "published": 0.17,
        "table": "Table 11",
        "param": "M=0.1, η=ζ=0.01, Rand periodic",
        "res": 128,
    },
    "M01_Eta01": {
        ...
        "published": 0.36,
        "table": "Table 11",
        "param": "M=0.1, η=ζ=0.1, Rand periodic",
        "res": 128,
    },
    "M01_inviscid": {
        ...
        "published": 0.28,
        "table": "Table 11",
        "param": "M=0.1, inviscid, Rand periodic",
        "res": 512,
    },
    "M10_Eta001": {
        ...
        "published": 0.096,
        "table": "Table 12",
        "param": "M=1.0, η=ζ=0.01, Rand periodic",
        "res": 128,
    },
    "M10_Eta01": {
        ...
        "published": 0.098,
        "table": "Table 12",
        "param": "M=1.0, η=ζ=0.1, Rand periodic",
        "res": 128,
    },
    "M10_inviscid": {
        ...
        "published": 0.35,
        "table": "Table 11",
        "param": "M=1.0, inviscid, Rand periodic",
        "res": 512,
    },
    "M01_Turb": {
        ...
        "published": 0.16,
        "table": "Table 12",
        "param": "M=0.1, inviscid Turb, periodic",
        "res": 512,
    },
    "M10_Turb": {
        ...
        "published": 0.13,
        "table": "Table 12",
        "param": "M=1.0, inviscid Turb, periodic",
        "res": 512,
    },
}
...
@app.function(
    gpu="A100",
    image=image,
    volumes={"/results": volume, "/data": data_vol},
    timeout=86400,
    memory=131072,
)
def train(config_key: str):
    ...
    cfg = CFD_CONFIGS[config_key]
    OUT = f"/results/2dcfd_{config_key}"
    ...
    print(f"2D CFD config: {config_key} — {cfg['param']}", flush=True)
    print(f"Published FNO nRMSE: {cfg['published']} ({cfg['table']})", flush=True)
    ...
    INIT_STEP = 5
    NOISE_STD = 5e-3
    # Adaptive splits: 512² datasets have only 1000 samples, 128² have 10000
    N_total_raw = None  # set after loading
    _N_TRAIN_128, _N_VAL_128, _N_TEST_128 = 8000, 1000, 1000
    _N_TRAIN_512, _N_VAL_512, _N_TEST_512 = 800, 100, 100
    LOG_EPS = 1e-6
    ...
    # ═══════════════════════════════════════════════════════════════
    # Download to persistent volume (survives timeouts, avoids re-download)
    data_vol.reload()
    hdf5_path = f"/data/{cfg['filename']}"
    darus_url = f"https://darus.uni-stuttgart.de/api/access/datafile/{cfg['darus_id']}"
    if not os.path.exists(hdf5_path) or os.path.getsize(hdf5_path) < 1_000_000_000:
        if os.path.exists(hdf5_path):
            os.remove(hdf5_path)
        print(f"Downloading {cfg['filename']} to persistent volume...", flush=True)
        subprocess.run(
            [
                "aria2c",
                "-x",
        ...
        data_vol.commit()
        print("Data saved to persistent volume", flush=True)
    else:
        print("Data already on volume (skipping download)", flush=True)
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        raw_res = cfg["res"]
        # Downsample 512→128 during loading (stride=4) to avoid OOM on 512² datasets
        if raw_res == 512:
            s = 4  # reduced_resolution=4 matches PDEBench protocol
            print(f"Loading 512² data with stride={s} → 128²", flush=True)
            rho = np.array(f["density"][:, :, ::s, ::s], dtype=np.float32)
            vx = np.array(f["Vx"][:, :, ::s, ::s], dtype=np.float32)
            vy = np.array(f["Vy"][:, :, ::s, ::s], dtype=np.float32)
            prs = np.array(f["pressure"][:, :, ::s, ::s], dtype=np.float32)
            x_c = np.array(f["x-coordinate"][::s], dtype=np.float32)
            y_c = np.array(f["y-coordinate"][::s], dtype=np.float32)
        else:
            rho = np.array(f["density"], dtype=np.float32)
    ...
    N, T, H, W = rho.shape
    # Set adaptive splits based on actual sample count
    if N >= 10000:
        N_TRAIN, N_VAL, N_TEST = _N_TRAIN_128, _N_VAL_128, _N_TEST_128
    else:
        N_TRAIN, N_VAL, N_TEST = _N_TRAIN_512, _N_VAL_512, _N_TEST_512
    print(f"Loaded: N={N}, T={T}, H={H}, W={W} (raw_res={raw_res})", flush=True)
