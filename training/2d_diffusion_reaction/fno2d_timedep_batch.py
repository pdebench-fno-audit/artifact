"""
Wave 5: 2D Time-Dependent PDEs — Autoregressive FNO2d
======================================================
Test 26: 2D Diffusion-Reaction (2 channels, 128×128×101)
Test 27: 2D Shallow Water / radial dam break (1 channel, 128×128×101)

Architecture: 2D FNO with SpectralConv2d + autoregressive rollout.
Input: INIT_STEP frames → predict next frame → feed back.
Protocol: train/val/test from 1000 samples (800/100/100 — smaller dataset).
Normalized MSE loss, stratified val.
"""
...
app = modal.App("fno-wave5-2d-timedep")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for xx, yy, grd in train_loader:
            ...
            for t in range(INIT_STEP, T_TOTAL):
                ...
                per_mse = ((pf - tf) ** 2).mean(dim=1)
                per_rms2 = (tf**2).mean(dim=1) + 1e-8
                loss = loss + (per_mse / per_rms2).mean()
